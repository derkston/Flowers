...
export const GoogleAuth = () => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      console.error('Ошибка входа через Google:', authError.code, authError.message);
      if (authError.code === 'auth/popup-closed-by-user') {
        setErrorMessage('Вход отменен пользователем.');
      } else if (authError.code === 'auth/cancelled-popup-request') {
        setErrorMessage('Окно входа уже открыто.');
      } else if (authError.code === 'auth/account-exists-with-different-credential') {
        setErrorMessage(`Аккаунт с таким email уже существует, но с другим провайдером. Пожалуйста, войдите, используя другой метод.`);
      } else {
        setErrorMessage(`Ошибка входа через Google: ${authError.message}`);
      }
    }
  };
  ...
  return (
    <div>
      ...
      <svg className='social_icon' onClick={handleGoogleSignIn} width="60px" height="60px" viewBox="0 0 256 262" version="1.1" xmlns="http://www.w3.org/2000/svg"  preserveAspectRatio="xMidYMid">
      <g>
